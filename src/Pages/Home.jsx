...
export class Home extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      topProductParkList: null,
      loadingTopProductParkList: false,
      searchByObjectsFiltersKeys: null,
      searchByObjectsFilters: [],
      productParkTree: null,
      loadingProductParkTree: false,
      productParkTreeDiagram: null,
      productParkTreeDiagramLoading: false,
  ...
  async loadTopProductParkList() {
    ...
    const data = await CommunicationWithServer.GetParentInjectionOutList(
      this.props.is_injection_in_classification
    )

    if (data != null) {
      this.setState({
        topProductParkList: data,
        selectedTopProductParkId: data[0].id
      })
    }
  ...
  async enterAndLoadProductParkTree() {
    this.setState({
      searchByObjectsFiltersKeys: null,
      productParkTree: null,
      loadingProductParkTree: true
    })

    const data = this.props.is_injection_in_classification
      ? await CommunicationWithServer.GetInjectionOutTreeTable(
          this.state.selectedTopProductParkId,
          this._child.current.getCheckedArray()
        )
      : await CommunicationWithServer.GetInjectionInTreeTable(
          this.state.selectedTopProductParkId,
          this._child.current.getCheckedArray()
        )
    ...
    if (data.node_edge_dictionary != null) {
      ...
      this.state.middle_classification__nodes =
        data.middle_classification__nodes
  ...
  async renderProductParkDiagramArea() {
    ...
    this.setState({
      productParkTreeDiagram: (
        <MyDiagram
          left_to_right={this.props.is_injection_in_classification}
          table={this.state.productParkTree}
          diagramStructureKey={this.state.productParkTreeDiagramStructureKey}
          searchByObjectsFilters={this.state.searchByObjectsFilters}
        />
      )
    })
  ...
  render() {
    let topProductParkSelectList =
      !this.state.loadingTopProductParkList &&
      this.state.topProductParkList != null
        ? this.state.topProductParkList.map(topProductPark => (
            <option value={topProductPark.id}>
              {topProductPark.name} ({topProductPark.id})
            </option>
          ))
        : null
    let productParkTreeDiagram =
      !this.state.loadingProductParkTree &&
      !this.state.productParkTreeDiagramLoading &&
      this.state.productParkTreeDiagram != null
    ...
    const radioGroupOnClick = key =>
      this.setState({ productParkTreeDiagramStructureKey: key })
        ...
        <Container fluid>
          <Row>
            <Col xs="auto">
              <Form>
                ...
                <Form.Group>
                  {
                    <CheckboxTreeWidget
                      ref={this._child}
                      nodes={this.state.checkboxTreeNodes}
                    />
                  }
                </Form.Group>
                ...
                <Button
                  ...
                  disabled={
                    this.state.loadingProductParkTree ||
                    this.state.selectedTopProductParkId <= 0 ||
                    this.state.productParkTreeDiagramStructureKey == null
                  }
            ...
            <Col xs="auto">
                          ...
                          {this.state.searchByObjectsFiltersKeys.map(key => (
                            <Col className="mb-3">
                              <Form.Group>
                                <CustomInputDropdown
                                  placeholder={key}
                                  updateArraySearchByKey={
                                    update_array_search_by_key
                                  }
                                  value={this.state.searchByObjectsFilters}
                                  objectsToDropdown={
                                    this.state.middle_classification__nodes[key]
                                  }
                                />
                              </Form.Group>
                            </Col>
                          ...
                          <Col md="auto" className="mb-3">
                            <Form.Group style={{ height: '100%' }}>
                              <Button
                                ...
                                type="button"
                                style={{
                                  backgroundColor: '#038e64',
                                  height: '100%'
                                }}
                                onClick={() =>
                                  this.setState({
                                    productParkTreeDiagramLoading: true
                                  })
                                }
                                disabled={
                                  this.state.productParkTreeDiagramLoading
              ...
              <Row>{productParkTreeDiagram}</Row>
            </Col>
          </Row>
        </Container>
      </div>
